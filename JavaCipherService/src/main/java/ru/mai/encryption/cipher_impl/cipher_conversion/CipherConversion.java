package ru.mai.encryption.cipher_impl.cipher_conversion;

import ru.mai.encryption.cipher_interface.ICipherConversion;
...
public class CipherConversion implements ICipherConversion {
    ...
    public byte[] apply(byte[] bytes, byte[] roundKey) {
        ...
        for (int i = 1; i <= S_BLOCKS.length; i++) {
            byte blockBits = (byte) BitsUtil.getFirstNBits(xorKeyAndRight, 6);
            byte col = (byte) ((blockBits & (((1 << 4) - 1) << 1)) >> 1);
            byte row = (byte) (((blockBits & (1 << 5)) >> 4) | (blockBits & 1));

            result |= (((long) S_BLOCKS[i - 1][row][col]) << (Long.SIZE - (Byte.SIZE / 2 * i)));
            xorKeyAndRight <<= 6;
        }
