...
@AllArgsConstructor
public class DEAL128KeyGenerate {
    ...
    public byte[][] generate() {
        ...
        keys[2] = desCipher.encrypt(BytesUtil.xor(BytesUtil.xor(key1, SHIFTS_BIT[0]), keys[1]));
        keys[3] = desCipher.encrypt(BytesUtil.xor(BytesUtil.xor(key2, SHIFTS_BIT[1]), keys[2]));
        keys[4] = desCipher.encrypt(BytesUtil.xor(BytesUtil.xor(key1, SHIFTS_BIT[2]), keys[3]));
        keys[5] = desCipher.encrypt(BytesUtil.xor(BytesUtil.xor(key2, SHIFTS_BIT[3]), keys[4]));
